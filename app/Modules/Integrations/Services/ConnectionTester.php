...
class ConnectionTester
{
    ...
    private function testLlm(IntegrationConfig $config): array
    {
        ...
        if (str_contains($config->provider, 'openai') || str_starts_with($apiKey, 'sk-or-')) {
            ...
            $headers = $isOpenRouter ? ['HTTP-Referer' => config('app.url'), 'X-Title' => config('app.name')] : [];
            $model = $isOpenRouter ? 'openrouter/auto' : 'gpt-4o-mini';
            ...
            return $resp->successful()
                ? ['ok' => true,  'message' => $isOpenRouter ? 'OpenRouter connection successful.' : 'OpenAI connection successful.']
                : ['ok' => false, 'message' => $resp->json()['error']['message'] ?? ($isOpenRouter ? 'OpenRouter error.' : 'OpenAI error.')];
        }
